...
async def test_category(test_user_token, test_server):
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        return category_response_data[0]
...
async def test_update_category(client: AsyncClient, test_user_token, test_server, test_category):
    ...
    response = await client.patch(f"/api/v0/category/{test_server['id']}/{category_id}", json=update_data,
                                  headers=headers)
...
async def test_create_category_invalid_name(client: AsyncClient, test_user_token, test_server):
    ...
    for name in invalid_names:
        response = await client.post(
            f"/api/v0/category/{test_server['id']}", json={"name": name}, headers=headers
        )
...
async def test_update_non_existent_category(client: AsyncClient, test_user_token, test_server):
    ...
    response = await client.patch(f"/api/v0/category/{test_server['id']}/{non_existent_id}", json=update_data,
                                  headers=headers)
...
async def test_create_category_without_permission(client: AsyncClient, test_user_token2, test_server):
    ...
    response = await client.post(
        f"/api/v0/category/{test_server['id']}", json=category_data, headers=headers
    )
